...
def search_game_data(game_name, platform="pc"):
  """
  Searches for game data using a basic web request.

  Args:
      game_name: The name of the game to search for.
      platform: The platform (defaults to "pc").

  Returns:
      A dictionary containing game data (if found), or None if not found.
  """
  url = f"https://store.epicgames.com/en-US/search?q={game_name}&sort=relevance&sortDir=DESC&count=1&platform={platform}"
  response = requests.get(url)
  soup = BeautifulSoup(response.content, "html.parser")

  # Implement your logic to parse game data from the HTML structure (replace with actual selectors)
  # ... (Implement your parsing logic here)
  # ...

  # Example logic for parsing name and link (replace with appropriate selectors)
  game_result = soup.find("div", class_="css-nl4yft")  # Replace with appropriate selector
  if game_result:
    name = game_result.find("a", class_="css-8y332x").text.strip()  # Replace with appropriate selector
    link = game_result.find("a", class_="css-8y332x")["href"]  # Replace with appropriate selector
    return {"name": name, "link": link}
  else:
    return None

# Example usage
game_data = search_game_data("Grand Theft Auto V")
if game_data:
  print(f"Game Name: {game_data['name']}")
  print(f"Game Link: {game_data['link']}")
else:
  print("Game not found on Epic Games Store.")